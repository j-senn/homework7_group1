...
void OnePct::ComputeMeanAndDev(){
  ...
  for(iter_multimap = voters_done_voting_.begin();
    ...
    sum_of_wait_times_seconds.append(voter.GetTimeWaiting());
  }
...
void OnePct::RunSimulationPct2(int stations_count){
  ...
  while(!done){
	...
	// if they have already arrived and there are free stations
...
/*******************************************************************************
**/
string OnePct::ToStringVoterMap(string label, multimap<int, OneVoter> the_map){
  string s =  "\n" + label + " WITH ";
	
  s.append(Utils::Format(static_cast <int>(the_map.size()), 6) + " ENTRIES\n");
